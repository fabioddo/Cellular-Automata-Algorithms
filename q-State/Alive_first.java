...
public final class Alive_first extends Cell<String>{
...
//creazione cella iniziale
public Alive_first(final int x, final int y, final List<Reference> n)
{
	    super(x, y, n);
	    
	    Random rand=new Random();
	    this.value=rand.nextInt(Initiator.q/5)+1;
	    this.sum=0;
		this.count=0;
}
...
//generazione nuova cella
public Alive_first(final Cell<String> c, int valore)
{
  super(c);
...
{ 
	//implementazione regole del problema
	int val=this.value;
	...
	//eventuale passaggio di stato
	if(val>=1 && val<=Initiator.q/5)
	{
		this.sum=0;
		this.count=0;
		this.value=val;
		...
		return new Alive_second(this, val);
