...
public final class Alive_fifth extends Cell<String>{
...
public Alive_fifth(final int x, final int y, final List<Reference> n)
{
	    super(x, y, n);
	    
	    Random rand=new Random();
	    this.value=rand.nextInt(Initiator.q/5)+4*(Initiator.q/5)+1;
	    this.sum=0;
		this.count=0;
}
...
public Alive_fifth(final Cell<String> c, int valore)
{
  super(c);
...
{ 
	...
	if(val>4*Initiator.q/5 && val<=Initiator.q)
	{
		this.sum=0;
		this.count=0;
		this.value=val;
	...
	else return new Alive_fourth(this, val);	
