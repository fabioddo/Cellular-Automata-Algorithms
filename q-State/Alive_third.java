...
public final class Alive_third extends Cell<String>{
...
public Alive_third(final int x, final int y, final List<Reference> n)
{
	    super(x, y, n);
	    
	    Random rand=new Random();
	    this.value=rand.nextInt(Initiator.q/5)+2*(Initiator.q/5)+1;
	    this.sum=0;
		this.count=0;
}
...
public Alive_third(final Cell<String> c, int valore)
{
  super(c);
...
{ 
	...
	if(val>Initiator.q/5 && val<=2*Initiator.q/5)
	{
		
		return new Alive_second(this, val);
		
	}
	else if(val>2*Initiator.q/5 && val<=3*Initiator.q/5)
	{
		...
		return new Alive_fourth(this, val);
