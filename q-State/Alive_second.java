...
public final class Alive_second extends Cell<String>{
...
public Alive_second(final int x, final int y, final List<Reference> n)
{
	    super(x, y, n);
	    
	    Random rand=new Random();
	    this.value=rand.nextInt(Initiator.q/5)+(Initiator.q/5)+1;
	    this.sum=0;
		this.count=0;
}
...
public Alive_second(final Cell<String> c, int valore)
{
  super(c);
...
{ 
	...
	if(val>=1 && val<=Initiator.q/5)
	{
		...
		return new Alive_first(this, val);
	
	}
	else if(val>Initiator.q/5 && val<=2*Initiator.q/5)
	{
